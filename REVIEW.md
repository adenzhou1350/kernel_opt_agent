...
| 机会与候选发现 | `opportunity init/add/rank/close/reopen`、`method validate/recommend`、`candidate init/add/run/promote` | 先从公式/递推得到 theory-to-kernel 依赖主线，再做条件收益上界及价值排序；哈希绑定死路关闭与显式重开；可迁移方法仅作先验；先实现 2--6 个聚焦候选，失败或不确定后再扩展，记录技术修复和预测残差 |
