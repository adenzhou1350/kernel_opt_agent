...
def validate_experiment_queue(queue: dict, workload: dict, errors: list[str]) -> None:
    ...
    if version == "experiment-request-queue-v3":
        ...
        require(
            len(active) == 1 or (not active and bool(resolved) and len(resolved) == len(requests)),
            "static-admissibility queue requires exactly one active gate, or every "
            "gate RESOLVED with APPLIED reconciliation",
            errors,
        )
    ...
    if queue_version == "experiment-request-queue-v3":
        ...
        if active:
            require(
                ...
                "static-admissibility contract: performance ranking is forbidden",
                errors,
            )
            require(
                len(active) == 1 and int(active[0].get("priority", -1)) == 0,
                "static-admissibility contract: exactly one priority-zero active gate is required",
                errors,
            )
        else:
            require(
                formula == "NO_PERFORMANCE_RANKING_RESOLVED_STATIC_GATE",
                "resolved static-admissibility contract requires terminal no-ranking policy",
                errors,
            )
            require(
                bool(resolved) and len(resolved) == len(queue.get("requests", []))
                and all(int(request.get("priority", -1)) == 0 for request in resolved),
                "zero active static gates requires every priority-zero gate RESOLVED "
                "with APPLIED reconciliation",
                errors,
            )
        for request in queue.get("requests", []):
