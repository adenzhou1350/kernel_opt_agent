...
from supervision_utils import artifact_path, validate_admissibility_contract, validate_decision_contract, validate_measurability_contract
...
def main() -> int:
    ...
    if queue.get("schema_version") not in {"experiment-request-queue-v2", "experiment-request-queue-v3"}:
        raise ValueError("legacy resource-centric requests are read-only; create a candidate-driven request")
    ...
    experiment = {
        ...
        "status": "PLANNED",
        "experiment_kind": request.get("experiment_kind", "PERFORMANCE_DECISION"),
    ...
    if queue_version == "experiment-request-queue-v3":
        experiment["admissibility_contract_identity"] = request["admissibility_contract"]
    else:
        experiment["decision_contract_identity"] = request["decision_contract"]
        experiment["measurability_contract_identity"] = request["measurability_contract"]
