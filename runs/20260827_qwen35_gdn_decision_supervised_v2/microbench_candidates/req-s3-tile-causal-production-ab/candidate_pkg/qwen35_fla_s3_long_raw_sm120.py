...
class S3PackedWorkspaceDecayRawKernelSm120(KeyedCompileMixin):
    def __init__(
        ...
        cu_seqlens_dtype,
        *,
        causal_tile_schedule: bool,
    ):
        self.dtype = dtype
        self.acc_dtype = cutlass.Float32
        self.cu_seqlens_dtype = cu_seqlens_dtype
        self.causal_tile_schedule = causal_tile_schedule
        self.manual_cache_key(
            "dtype", "cu_seqlens_dtype", "causal_tile_schedule"
        )
    ...
    def causal_qk_main(
        ...
        """Compute only causal 16x16 QK tiles, preserving K order.

        Four warps own the four query-row tiles.  Each warp visits key tiles in
        ascending order and issues only col_tile <= row_tile.  The S404 tail
        therefore executes (0,0), (1,0), (1,1); diagonal/tail elements remain
        masked below, so the BF16 score boundary is unchanged.
        """
        ...
        for col_tile in cutlass.range_constexpr(BT // 16):
            if (
                ...
                k_tile = k_tiles[None, None, cutlass.Int32(col_tile), 0]
                k_operand = self.load_b_aligned(
                    k_tile, qk_mma, lane, (16, D), False
                )
    ...
    def causal_score_v_accumulate(
        ...
        """Accumulate the same nonzero score-V terms in ascending K order.

        The existing D x 64 state fragment remains live.  The candidate builds
        a layout-only view whose four outer modes are the 16-token tiles.  The
        compile-time layout equality assertion is an admission gate: no
        register copy, shared-memory handoff or state-GEMM retile is allowed.
        """

        warp_idx = tidx // cutlass.Int32(32)
        lane = tidx % cutlass.Int32(32)
        v_tiles = cute.flat_divide(s_v, (16, 16))
        score_tiles = cute.flat_divide(s_qk, (16, 16))
        ...
        # This equality is deliberately stronger than equal element count.  A
        # failure means the current state accumulator is not a zero-copy
        # concatenation of 16-token MMA fragments and the experiment must stop
        # at compile admission; a scan/copy or shared roundtrip is forbidden.
        assert output.layout == output_tiles_layout
        output_tiles = cute.make_tensor(output.iterator, output_tiles_layout)
        for row_tile in cutlass.range_constexpr(BT // 16):
            if cutlass.Int32(row_tile * 16) < valid_len:
                tile_output = output_tiles[
                    None,
                    None,
                    None,
                    row_tile,
                    ...
                    if cutlass.const_expr(col_tile <= row_tile):
                        if cutlass.Int32(col_tile * 16) < valid_len:
                            ...
                            v_operand = self.load_a_aligned(
                                v_tile, o2_mma, lane, (16, 16), True
                            )
                            score_operand = self.load_b_aligned(
                                score_tile, o2_mma, lane, (16, 16), False
                            )
                            cute.gemm(
                                o2_mma,
                                tile_output,
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.causal_tile_schedule):
