...
#pragma unroll 1
  ...
  if ((lane == 0) && isfinite(value)) atomicAdd(output, value);
}
...
#pragma unroll
  ...
  if ((threadIdx.x & 31) == 0 && isfinite(sum)) atomicAdd(output, sum);
}
...
Options parse(int argc, char** argv) {
  ...
  std::map<std::string, int*> integers{{"device", &options.device}, {"grid", &options.grid}, {"block", &options.block}, {"repeats", &options.repeats}, {"batches", &options.batches}, {"samples", &options.samples}, {"stride", &options.stride}};
...
void launch(const Options& o, float* input, float* output, half* ha, half* hb) {
  if (o.variant == "zero") {
    ...
    if (o.variant == "r32") register_probe<8><<<o.grid, o.block>>>(input, output, o.repeats, 0);
    ...
    else register_probe<32><<<o.grid, o.block>>>(input, output, o.repeats, 1);
...
int main(int argc, char** argv) {
  try {
    ...
    CUDA_CHECK(cudaMalloc(&output, 4096));
    ...
    CUDA_CHECK(cudaMemset(output, 0, 4096));
    init_half<<<256, 256>>>(ha, 16 * 4096);
    init_half<<<256, 256>>>(hb, 16 * 4096);
    CUDA_CHECK(cudaDeviceSynchronize());
    for (int i = 0; i < 20; ++i) launch(o, input, output, ha, hb);
    CUDA_CHECK(cudaDeviceSynchronize());
    ...
    float sink = 0.f;
    CUDA_CHECK(cudaMemcpy(&sink, output, sizeof(float), cudaMemcpyDeviceToHost));
