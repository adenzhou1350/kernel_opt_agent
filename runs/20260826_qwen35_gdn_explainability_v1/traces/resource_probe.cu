...
#define CUDA_CHECK(expr) do { \
  ...
  if (status_ != cudaSuccess) throw std::runtime_error( \
      std::string(cudaGetErrorString(status_)) + " at " + #expr + ":" + std::to_string(__LINE__)); \
} while (0)
...
#pragma unroll 1
  for (int r = 0; r < repeats; ++r) {
#pragma unroll
    for (int i = 0; i < 8; ++i) payload[i] = fmaf(payload[i], 1.000000119f, 0.000001f);
  }
  float sum = 0.f;
#pragma unroll
...
Options parse(int argc, char** argv) {
  ...
  std::map<std::string, int*> integers{{"device", &options.device}, {"grid", &options.grid}, {"block", &options.block}, {"repeats", &options.repeats}, {"batches", &options.batches}, {"samples", &options.samples}, {"stride", &options.stride}, {"preheat-ms", &options.preheat_ms}, {"smem-bytes", &options.smem_bytes}};
...
void launch(const Options& o, float* input, float* output, half* ha, half* hb, cudaStream_t stream) {
  if (o.variant == "zero") {
    zero_probe<<<o.grid, o.block, 0, stream>>>(output);
  } else if (o.mode == "shared") {
    if (o.variant == "constant_broadcast") constant_probe<<<o.grid, o.block, 0, stream>>>(output, 0, o.repeats);
    else if (o.variant == "constant_divergent") constant_probe<<<o.grid, o.block, 0, stream>>>(output, 1, o.repeats);
    else if (o.variant == "global_request") global_request_probe<<<o.grid, o.block, 0, stream>>>(reinterpret_cast<float4*>(input), reinterpret_cast<float4*>(input + 65536), output, 65536 / 4, 0, std::max(1, o.repeats / 8));
    else shared_probe<<<o.grid, o.block, o.block * sizeof(float), stream>>>(input, output, o.stride, o.repeats);
  } else if (o.mode == "register") {
    if (o.variant == "alloc0") register_allocation_probe<0><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc32") register_allocation_probe<32><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc64") register_allocation_probe<64><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc96") register_allocation_probe<96><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc112") register_allocation_probe<112><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc116") register_allocation_probe<116><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc120") register_allocation_probe<120><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc124") register_allocation_probe<124><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "alloc128") register_allocation_probe<128><<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats);
    else if (o.variant == "shfl_dep") collective_probe<<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats, 0);
    else if (o.variant == "shfl_ilp4") collective_probe<<<o.grid, o.block, o.smem_bytes, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "r32") register_probe<8><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 0);
    else if (o.variant == "r64") register_probe<16><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 0);
    else if (o.variant == "r96") register_probe<24><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "r128") register_probe<32><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "n48") register_probe<48><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "n64") register_probe<64><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "n96") register_probe<96><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "n112") register_probe<112><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "n128") register_probe<128><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else if (o.variant == "n144") register_probe<144><<<o.grid, o.block, 0, stream>>>(input, output, o.repeats, 1);
    else throw std::runtime_error("unknown register variant: " + o.variant);
  } else if (o.mode == "compute") {
    if (o.variant == "tensor") tensor_probe<<<o.grid, o.block, 0, stream>>>(ha, hb, output, std::max(1, o.repeats / 8));
    else {
      int variant = o.variant == "fma_dep" ? 0 : o.variant == "fma_ilp4" ? 1 : o.variant == "sfu" ? 2 : 3;
      simt_compute_probe<<<o.grid, o.block, 0, stream>>>(input, output, variant, o.repeats);
    }
  } else if (o.mode == "memory") {
    size_t count = std::max<size_t>(1, o.bytes / sizeof(float4));
    global_request_probe<<<o.grid, o.block, 0, stream>>>(reinterpret_cast<float4*>(input), reinterpret_cast<float4*>(input + o.bytes / sizeof(float)), output, count, o.variant == "write", std::max(1, o.repeats / 16));
  } else if (o.mode == "sync") {
    sync_copy_probe<<<o.grid, o.block, o.block * sizeof(float), stream>>>(input, output, o.repeats, o.variant == "async");
  } else {
    throw std::runtime_error("unknown mode: " + o.mode);
  }
}

int register_occupancy_blocks_per_sm(const Options& o) {
  int blocks = -1;
#define QUERY_OCCUPANCY(function_name) \
  CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocks, function_name, o.block, o.smem_bytes))
  if (o.variant == "zero") {
    CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocks, zero_probe, o.block, 0));
  } else if (o.variant == "alloc0") QUERY_OCCUPANCY(register_allocation_probe<0>);
  else if (o.variant == "alloc32") QUERY_OCCUPANCY(register_allocation_probe<32>);
  else if (o.variant == "alloc64") QUERY_OCCUPANCY(register_allocation_probe<64>);
  else if (o.variant == "alloc96") QUERY_OCCUPANCY(register_allocation_probe<96>);
  else if (o.variant == "alloc112") QUERY_OCCUPANCY(register_allocation_probe<112>);
  else if (o.variant == "alloc116") QUERY_OCCUPANCY(register_allocation_probe<116>);
  else if (o.variant == "alloc120") QUERY_OCCUPANCY(register_allocation_probe<120>);
  else if (o.variant == "alloc124") QUERY_OCCUPANCY(register_allocation_probe<124>);
  else if (o.variant == "alloc128") QUERY_OCCUPANCY(register_allocation_probe<128>);
  else if (o.variant == "shfl_dep" || o.variant == "shfl_ilp4") QUERY_OCCUPANCY(collective_probe);
...
int main(int argc, char** argv) {
  try {
    ...
    if (o.mode == "register" && o.smem_bytes > 49152) {
#define SET_SMEM(function_name) CUDA_CHECK(cudaFuncSetAttribute(function_name, cudaFuncAttributeMaxDynamicSharedMemorySize, o.smem_bytes))
      SET_SMEM(register_allocation_probe<0>);
      SET_SMEM(register_allocation_probe<32>);
      SET_SMEM(register_allocation_probe<64>);
      SET_SMEM(register_allocation_probe<96>);
      SET_SMEM(register_allocation_probe<112>);
      SET_SMEM(register_allocation_probe<116>);
      SET_SMEM(register_allocation_probe<120>);
      SET_SMEM(register_allocation_probe<124>);
      SET_SMEM(register_allocation_probe<128>);
      SET_SMEM(collective_probe);
#undef SET_SMEM
    }
    const int occupancy_blocks_per_sm =
        o.mode == "register" ? register_occupancy_blocks_per_sm(o) : -1;
    ...
    do {
      for (int i = 0; i < 32; ++i) launch(o, input, output, ha, hb, stream);
    ...
    const bool graph_measurement = o.action == "measure";
    cudaGraph_t graph = nullptr;
    cudaGraphExec_t graph_exec = nullptr;
    if (graph_measurement) {
      CUDA_CHECK(cudaStreamBeginCapture(stream, cudaStreamCaptureModeThreadLocal));
      for (int batch = 0; batch < o.batches; ++batch) launch(o, input, output, ha, hb, stream);
      CUDA_CHECK(cudaStreamEndCapture(stream, &graph));
      CUDA_CHECK(cudaGraphInstantiateWithFlags(&graph_exec, graph, 0));
      for (int graph_warmup = 0; graph_warmup < 8; ++graph_warmup)
    ...
    std::cout << std::setprecision(9) << "{\"status\":\"PASS\",\"mode\":\"" << o.mode
              ...
              << ",\"measurement_launch_path\":\"" << (graph_measurement ? "cuda_graph_batch_external_events" : "direct") << "\""
              << ",\"block\":" << o.block << ",\"stride\":" << o.stride
              << ",\"dynamic_smem_bytes\":" << o.smem_bytes
