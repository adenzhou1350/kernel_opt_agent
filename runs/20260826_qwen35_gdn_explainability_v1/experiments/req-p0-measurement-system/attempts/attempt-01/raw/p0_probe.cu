...
__global__ void zero_work_kernel(uint32_t* sink) {
  if (blockIdx.x == 0 && threadIdx.x == 0) atomicAdd(sink, 1u);
}
...
__global__ void positive_work_kernel(uint32_t* sink, int repeats) {
  ...
  if (threadIdx.x == 0) atomicAdd(sink, value | 1u);
}
...
int main(int argc, char** argv) {
  try {
    ...
    CUDA_CHECK(cudaMalloc(&sink, sizeof(uint32_t)));
    CUDA_CHECK(cudaMemset(sink, 0, sizeof(uint32_t)));
    cudaStream_t stream;
    ...
    uint32_t host_sink = 0;
    CUDA_CHECK(cudaMemcpy(&host_sink, sink, sizeof(host_sink), cudaMemcpyDeviceToHost));
