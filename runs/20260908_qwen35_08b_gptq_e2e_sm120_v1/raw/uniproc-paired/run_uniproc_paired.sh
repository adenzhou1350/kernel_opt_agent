...
output_root="$experiment/uniproc-paired-v1"
...
run_arm() {
  ...
  local cache_root="$experiment/cache/uniproc-paired-v1-repeat-${repeat}-${arm}"
  [[ ! -e "$output_dir" ]]
  [[ ! -e "$cache_root" ]]
...
# Fixed balanced order: each arm runs once first and once second.
run_arm 0 control 1
run_arm 0 uniproc 0
run_arm 1 uniproc 0
run_arm 1 control 1

echo "paired_uniproc_complete"
