#!/usr/bin/env python3
"""Seal the revised PASS-only N2 static admission experiment."""

from __future__ import annotations

import ast
import hashlib
import json
from datetime import datetime, timezone
from pathlib import Path


ROOT = Path("/workspace/dance/qwen35/kernel_opt_agent")
RUN = ROOT / "runs/20260827_qwen35_gdn_layout_replan_v3"
REQUEST = "req-n2-layout-view-static-v2"
CANDIDATE = RUN / "microbench_candidates" / REQUEST
EXPERIMENT = RUN / "experiments" / REQUEST / "experiment.json"
CONTRACT = RUN / "models/admissibility_contracts/n2_zero_copy_layout_v1.json"
SOURCE_NAMES = (
    "analyze.py", "clean_build.py", "common.py", "correctness.py",
    "layout_proof.py", "measure.py", "static_audit.py", "warmup.py",
)
PRODUCTION = (
    Path("/workspace/dance/qwen35/flashinfer/flashinfer/gdn_kernels/delta_rule_dsl/qwen35_fla_s3_short_raw_sm120.py"),
    Path("/workspace/dance/qwen35/flashinfer/flashinfer/gdn_kernels/delta_rule_dsl/qwen35_fla_s3_long_raw_sm120.py"),
)
CUTLASS_ROOT = Path(
    "/workspace/dance/qwen35/.venv-cu13/lib/python3.12/site-packages/"
    "nvidia_cutlass_dsl/dsl_packages/cutlass/cute"
)
CUTLASS_LAYOUT = tuple(CUTLASS_ROOT / f"{name}.py" for name in ("atom", "core", "tensor"))
STATIC_TOOLCHAIN = (
    CUTLASS_ROOT / "nvgpu/warp/mma.py",
    CUTLASS_ROOT.parent / "base_dsl/ast_preprocessor.py",
    CUTLASS_ROOT.parent / "base_dsl/ast_helpers.py",
    CUTLASS_ROOT.parent / "_mlir/_mlir_libs/_cutlass_ir.cu13.cpython-312-x86_64-linux-gnu.so",
    Path("/usr/local/cuda/bin/ptxas"),
    Path("/usr/local/cuda/bin/cuobjdump"),
)
LEGACY_NEGATIVE = (
    RUN / "traces/static_admissibility_revision_02/legacy_attempt2_short_source.py"
)


def digest(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def identity(path: Path) -> dict:
    return {"path": str(path), "sha256": digest(path)}


def write(path: Path, data: object) -> None:
    temporary = path.with_suffix(path.suffix + ".tmp")
    temporary.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
    temporary.replace(path)


def call_name(node: ast.Call) -> str:
    function = node.func
    if isinstance(function, ast.Subscript):
        function = function.value
    parts = []
    while isinstance(function, ast.Attribute):
        parts.append(function.attr)
        function = function.value
    if isinstance(function, ast.Name):
        parts.append(function.id)
    return ".".join(reversed(parts))


def source_audit() -> dict:
    launch_sites = []
    compile_sites = []
    subprocess_sites = []
    forbidden = []
    for name in SOURCE_NAMES:
        path = CANDIDATE / name
        tree = ast.parse(path.read_text(), filename=str(path))
        for node in ast.walk(tree):
            if not isinstance(node, ast.Call):
                continue
            qualified = call_name(node)
            if qualified == "launch" or qualified.endswith(".launch"):
                launch_sites.append((name, node.lineno))
            if qualified == "cute.compile":
                compile_sites.append((name, node.lineno))
            if qualified == "subprocess.run":
                subprocess_sites.append((name, node.lineno))
            if qualified in {
                "time.perf_counter", "timeit.timeit", "torch.cuda.Event",
                "torch.cuda.synchronize", "cudaLaunchKernel", "cudaLaunchKernelExC",
            }:
                forbidden.append((name, qualified, node.lineno))
            if isinstance(node.func, ast.Name) and node.func.id == "compiled":
                forbidden.append((name, "compiled_callable_invocation", node.lineno))
    if forbidden:
        raise RuntimeError(f"forbidden dynamic/timing calls: {forbidden}")
    if len(compile_sites) != 1 or compile_sites[0][0] != "clean_build.py":
        raise RuntimeError(f"unexpected cute.compile sites: {compile_sites}")
    if len(launch_sites) != 1 or launch_sites[0][0] != "layout_proof.py":
        raise RuntimeError(f"launch must exist only in uninvoked proof __call__: {launch_sites}")
    if len(subprocess_sites) != 1 or subprocess_sites[0][0] != "static_audit.py":
        raise RuntimeError(f"only static cuobjdump subprocess is allowed: {subprocess_sites}")
    audit_text = (CANDIDATE / "static_audit.py").read_text()
    if '"/usr/local/cuda/bin/cuobjdump"' not in audit_text or '"--dump-sass"' not in audit_text:
        raise RuntimeError("static_audit subprocess differs from exact cuobjdump whitelist")
    all_text = "\n".join((CANDIDATE / name).read_text() for name in SOURCE_NAMES)
    for forbidden_text in ("StaticRejectMarker", "PREDICATE_REJECT:"):
        if forbidden_text in all_text:
            raise RuntimeError(f"PASS-only source contains forbidden reject lifecycle: {forbidden_text}")
    for required_text in (
        "verify_static_toolchain_sources()",
        "verify_legacy_negative_source()",
        "legacy_negative_ast_binding",
        "FORBIDDEN_RUNTIME_ASSERT_TOKENS",
        "audit_no_runtime_assert_or_trap",
    ):
        if required_text not in all_text:
            raise RuntimeError(f"required static proof closure is absent: {required_text}")
    return {
        "status": "PASS",
        "compile_sites": compile_sites,
        "compiled_launch_sites": launch_sites,
        "subprocess_sites": subprocess_sites,
        "forbidden_dynamic_or_timing_calls": [],
        "lifecycle": "PASS_ONLY_INVALID",
    }


def main() -> None:
    experiment = json.loads(EXPERIMENT.read_text())
    contract = json.loads(CONTRACT.read_text())
    if experiment.get("status") != "PLANNED" or experiment.get("request_id") != REQUEST:
        raise RuntimeError("experiment scaffold is not the expected PLANNED request")
    if contract.get("lifecycle") != "PASS_ONLY_INVALID":
        raise RuntimeError("contract is not PASS_ONLY_INVALID")
    if any("REJECT" in item.get("outcome", "") for item in contract.get("outcomes", [])):
        raise RuntimeError("PASS-only contract unexpectedly authorizes candidate rejection")

    production_expected = {
        "qwen35_fla_s3_short_raw_sm120.py": "2b61b0da46b13802fcc75620fe7f87fe50d4de6660259327ee08696b0b83929f",
        "qwen35_fla_s3_long_raw_sm120.py": "2b647e3971a36929a2239c1ade1b4afec33894e0cb6ec638d6b0b046871e149f",
    }
    cutlass_expected = {
        "atom.py": "41a61c0dcc44eb1f852db9e6cf1a42d6098f66e42c7059015d1d04879d3de848",
        "core.py": "035c764686c4e5a94c1f2432b55ba2e6cc572a27db4d857278ccd785a6f7f6f3",
        "tensor.py": "c0707b064be449f726ffb895a6a745ffa9ea7cc814da3b97f86ce8c1597b6a27",
    }
    static_toolchain_expected = {
        "mma.py": "afd7a67d9ecc87494929f1099d2e2da42371034ca55a8db5a7052e25c0d7c773",
        "ast_preprocessor.py": "97c9acb67af2bf2fb01d73c39556e906954ede8251a42b73315d0330d3c554c8",
        "ast_helpers.py": "5b357a0084b72b6c9cc6391921e8a5d08e743652f2b60d649a88bf50957abcfc",
        "_cutlass_ir.cu13.cpython-312-x86_64-linux-gnu.so": "e20f9ddef63b8e4257fc26d87b80d15f81cb62104d84e7a6a7d079d913559a75",
        "ptxas": "7fdd01a4cf50e30746da98989c9272a907f491e6fd7fecfda14642e4375f88fb",
        "cuobjdump": "8690c347b8b4ce8ce0491a2fd10de9c99e02ec3600d1c7e101cf27719500a6ad",
    }
    for path in PRODUCTION:
        if digest(path) != production_expected[path.name]:
            raise RuntimeError(f"production identity changed before seal: {path}")
    for path in CUTLASS_LAYOUT:
        if digest(path) != cutlass_expected[path.name]:
            raise RuntimeError(f"CUTLASS layout implementation changed before seal: {path}")
    for path in STATIC_TOOLCHAIN:
        if digest(path) != static_toolchain_expected[path.name]:
            raise RuntimeError(f"CUTLASS/CUDA static toolchain changed before seal: {path}")
    if digest(LEGACY_NEGATIVE) != "089272013fc3e1f009d371b639ac50e72805b6a47136ccb5879fff3330f716f6":
        raise RuntimeError("immutable legacy attempt-2 source changed before seal")

    audit = source_audit()
    python = "/workspace/dance/qwen35/.venv-cu13/bin/python"
    commands = {
        phase: [[python, str(CANDIDATE / f"{phase}.py"), "--run", str(RUN)]]
        for phase in ("clean_build", "static_audit", "correctness", "warmup", "measure", "analyze")
    }
    source_identities = [identity(CANDIDATE / name) for name in SOURCE_NAMES]
    source_identities.extend(identity(path) for path in PRODUCTION)
    source_identities.extend(identity(path) for path in CUTLASS_LAYOUT)
    source_identities.extend(identity(path) for path in STATIC_TOOLCHAIN)
    source_identities.append(identity(LEGACY_NEGATIVE))
    experiment.update({
        "status": "MATERIALIZED",
        "sealed_at": datetime.now(timezone.utc).isoformat(),
        "commands": commands,
        "source": {
            "mode": "CREATE_RUN_LOCAL",
            "package_id": None,
            "candidate_path": str(CANDIDATE),
            "identities": source_identities,
        },
    })
    experiment["model_update_contract"]["summary_fields"] = [
        "static_admissibility",
        "n2_disposition",
        "latency_model_update_authorized",
        "performance_top_two_update_authorized",
        "cuda_kernel_launches",
        "gpu_performance_samples",
    ]
    evidence_paths = [
        RUN / "experiments" / REQUEST / "catalog_query_receipt.json",
        CONTRACT,
        RUN / "models/production_constructor_bindings.json",
        RUN / "traces/preapproval_veto_revision_02/review_receipt.json",
        RUN / "traces/preapproval_veto_revision_03/review_receipt.json",
        RUN / "traces/preapproval_veto_revision_04/review_receipt.json",
        RUN / "traces/static_admissibility_revision_02/admissibility_status_initial.json",
        RUN / "traces/static_admissibility_revision_02/revision_receipt.json",
        RUN / "traces/experiment_ranking_receipt.json",
    ]
    experiment["evidence"] = [identity(path) for path in evidence_paths]
    write(EXPERIMENT, experiment)

    queue_path = RUN / "models/experiment_queue.json"
    queue = json.loads(queue_path.read_text())
    request = next(item for item in queue["requests"] if item["request_id"] == REQUEST)
    request["status"] = "PLANNED"
    request["materialized_experiment"] = {**identity(EXPERIMENT), "status": "MATERIALIZED"}
    receipt = RUN / "experiments" / REQUEST / "catalog_query_receipt.json"
    queue["catalog_snapshot"] = {
        "status": "CURRENT",
        "catalog_identity": identity(ROOT / "microbench/catalog.json"),
        "request_receipts": [identity(receipt)],
    }
    write(queue_path, queue)

    seal_path = RUN / "traces/static_admissibility_experiment_seal_v3.json"
    write(seal_path, {
        "schema_version": "static-admissibility-experiment-seal-v3",
        "sealed_at": experiment["sealed_at"],
        "request_id": REQUEST,
        "revision": 2,
        "lifecycle": "PASS_ONLY_INVALID",
        "experiment_identity": identity(EXPERIMENT),
        "queue_identity": identity(queue_path),
        "contract_identity": identity(CONTRACT),
        "source_identities": source_identities,
        "source_audit": audit,
        "process_launches": 6,
        "compiled_callable_invocations_authorized": 0,
        "cuda_kernel_launches_authorized": 0,
        "gpu_performance_samples_authorized": 0,
        "candidate_rejection_authorized": False,
    })
    print(json.dumps({
        "status": "PASS",
        "seal": str(seal_path),
        "experiment_sha256": digest(EXPERIMENT),
    }, sort_keys=True))


if __name__ == "__main__":
    main()
