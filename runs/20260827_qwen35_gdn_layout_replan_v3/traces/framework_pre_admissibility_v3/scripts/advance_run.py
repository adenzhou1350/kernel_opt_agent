...
def validate_experiment_queue(queue: dict, workload: dict, errors: list[str]) -> None:
    ...
    require(version in {"experiment-request-queue-v1", "experiment-request-queue-v2"}, "experiment queue: invalid schema_version", errors)
    ...
    for index, request in enumerate(requests if isinstance(requests, list) else []):
        ...
        sensitivity_fields = (
            ("candidate_specific_decision_value_us", "decision_flip_probability", "expected_uncertainty_reduction", "experiment_cost")
            if version == "experiment-request-queue-v2"
            else ("max_weighted_benefit_us", "critical_path_probability", "uncertainty", "experiment_cost")
        )
...
def validate_strict_ranking(run: Path, queue: dict, errors: list[str]) -> None:
    ...
    if queue.get("schema_version") == "experiment-request-queue-v2":
        expected_formula = "candidate_specific_decision_value_us * decision_flip_probability * expected_uncertainty_reduction / experiment_cost_weight"
        ...
        for request in queue.get("requests", []):
            ...
            try:
                ...
                require(math.isclose(value, float(sensitivity["candidate_specific_decision_value_us"]), rel_tol=1e-12), f"candidate-supervised request {request.get('request_id')}: stale decision value", errors)
