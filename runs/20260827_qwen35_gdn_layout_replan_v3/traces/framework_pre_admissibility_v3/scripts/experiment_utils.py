...
def validate_materialized_experiment(path: Path, run: Path) -> list[str]:
    ...
    for field in ("decision_contract_identity", "measurability_contract_identity"):
...
def validate_benchmark_result(
    ...
    *,
    ...
    if not isinstance(data.get("raw_samples"), list) or len(data["raw_samples"]) < 9:
        errors.append("benchmark result: at least nine raw samples are required")
