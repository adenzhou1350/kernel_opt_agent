...
def production_ast_binding(name: str, path: Path) -> dict:
    ...
    if name == "short":
        ...
        guards = [
            ...
            and ast.unparse(node.test) == "tidx < MAIN_THREADS"
            and any(
                isinstance(call, ast.Call)
                and isinstance(call.func, ast.Attribute)
                and call.func.attr == "output_main"
                for call in ast.walk(node)
            )
        ]
        if len(guards) != 1:
            raise RuntimeError("INFRA_FAILURE: tidx<MAIN_THREADS does not uniquely dominate output_main")
        active_guard = canonical(guards[0].test)
    else:
        class_name, producer_method = "S3PackedWorkspaceDecayRawKernelSm120", "__call__"
        threads = exact_constant(tree, "THREADS")
...
def validate_ast_triplet(short: dict, long: dict, proof: dict) -> dict:
...
class N2StaticLayoutProof:
    ...
    def proof_body(self, g_sink: cute.Tensor, tidx: cutlass.Int32):
        ...
        legacy_fragment = legacy_thr.make_fragment_C(legacy_thr.partition_shape_C((N_TILE, N_TILE)))
        legacy_storage = cute.cosize(legacy_fragment.layout)
        legacy_appended_layout = cute.append(
            legacy_fragment.layout,
            cute.make_layout((BT // N_TILE,), stride=(legacy_storage,)),
        )
        assert output.layout != legacy_appended_layout
        legacy_tv = legacy_mma.tv_layout_C_tiled
