#!/usr/bin/env python3
"""Create a schema-valid static result with PASS and REJECT dispositions."""
...
def main() -> int:
    ...
    if outcome == "PASS":
        ...
        allowed = ["N2 static layout-view admissibility"]
    elif outcome == "REJECT":
        disposition = "REJECTED_STATIC_LAYOUT"
        allowed = ["N2 static layout-view rejection", f"predicate_id={manifest['predicate_id']}"]
    else:
        raise RuntimeError(f"unrecognized controlled outcome: {outcome}")
    ...
    result = {
        ...
        "summary": {
            ...
            "survivor_set_update_authorized": outcome in {"PASS", "REJECT"},
        ...
        "static_evidence": {
            ...
            "resource_usage": {
                ...
                "logical_backing_transport_allocations": 0, "cuda_kernel_launches": 0,
            },
        },
        ...
        "validity": {
            "status": "VALID",
            "dce_guard": "live O1 producer to four scoreV C/D consumers to typed global sink; proof callable not invoked",
            "known_pollution": [], "claims_allowed": allowed,
            "claims_forbidden": [
                "GPU latency", "speedup", "numerical correctness", "K-loop accumulation order",
