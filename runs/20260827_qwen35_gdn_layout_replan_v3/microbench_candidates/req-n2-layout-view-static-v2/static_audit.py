...
def audit_no_runtime_assert_or_trap(text: str, artifact: str) -> dict:
    lowered = text.lower()
    matches = [token for token in FORBIDDEN_RUNTIME_ASSERT_TOKENS if token in lowered]
    if matches:
        raise RuntimeError(
            f"runtime assert/trap path found in {artifact}: {matches}"
        )
    return {
        "artifact": artifact,
        "forbidden_tokens": list(FORBIDDEN_RUNTIME_ASSERT_TOKENS),
        "matched_tokens": [],
        "status": "PASS",
    }
...
def main() -> int:
    ...
    if manifest.get("static_toolchain_sources") != verify_static_toolchain_sources():
        raise RuntimeError("CUTLASS/CUDA static toolchain changed after clean build")
    if manifest.get("legacy_negative_source") != verify_legacy_negative_source():
        raise RuntimeError("immutable legacy attempt-2 source changed after clean build")
    ...
    sass_identities = {}
    binary_identities = {}
    runtime_assert_trap_audits = {}
    for name in ("short", "long"):
        ptx_identity = manifest["artifacts"][name]["ptx"]
        ptx = Path(ptx_identity["path"])
        if sha256(ptx) != ptx_identity["sha256"]:
            raise RuntimeError(f"PTX identity changed: {name}")
        runtime_assert_trap_audits[f"{name}.ptx"] = audit_no_runtime_assert_or_trap(
            ptx.read_text(errors="replace"), f"{name}.ptx"
        )
        ...
        runtime_assert_trap_audits[f"{name}.sass"] = audit_no_runtime_assert_or_trap(
            completed.stdout, f"{name}.sass"
        )
    ...
    dump(audit_path, {
        ...
        "runtime_assert_trap_audits": runtime_assert_trap_audits,
        "assertion_semantics": (
            "all proof assertions must discharge during frozen CuTe/MLIR codegen; "
            "runtime assert/trap paths are forbidden because proof kernels are never launched"
        ),
