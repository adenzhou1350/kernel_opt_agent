...
def main() -> int:
    ...
    toolchain_bindings = verify_static_toolchain_sources()
    legacy_identity = verify_legacy_negative_source()
    production_ast = {
    ...
    legacy_ast = legacy_negative_ast_binding(LEGACY_NEGATIVE_SOURCE, proof=False)
    proof_negative_ast = legacy_negative_ast_binding(
        source_root / "layout_proof.py", proof=True
    )
    ast_triplet = validate_ast_triplet(
        production_ast["short"], production_ast["long"], proof_ast,
        legacy_ast, proof_negative_ast,
    )
    ...
    dump(ast_report_path, {
        ...
        "static_toolchain_source_identities": toolchain_bindings,
        "legacy_negative_source_identity": legacy_identity,
        "production_bindings": production_ast,
        "proof_binding": proof_ast,
        "legacy_negative_binding": legacy_ast,
        "proof_negative_binding": proof_negative_ast,
    ...
    dump(manifest_path, {
        ...
        "static_toolchain_sources": toolchain_bindings,
        "legacy_negative_source": legacy_identity,
