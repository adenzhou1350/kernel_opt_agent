...
def compile_configuration(candidate_id: str, path_name: str, build: Path) -> dict:
    ...
    configuration_pre_sources = verify_bound_sources()
    ...
    from flashinfer.gdn_kernels.delta_rule_dsl.varlen_helper import (  # noqa: PLC0415
    ...
    runtime_import_origins = verify_loaded_module_origins(path_name)
    ...
    configuration_post_sources = verify_bound_sources()
    assert_same_source_snapshot(
        configuration_pre_sources, configuration_post_sources
    )
    return {
        "status": "PASS_CODEGEN",
        "candidate_id": candidate_id,
        ...
        "compile_toctou": {
            "pre_compile": configuration_pre_sources,
            "post_compile": configuration_post_sources,
            "pass": True,
        },
        "ptx": ptx,
        "cubin": cubin,
...
def main() -> int:
    ...
    try:
        ...
        pre_compile_sources = verify_bound_sources()
    ...
    try:
        post_control_sources = verify_bound_sources()
        assert_same_source_snapshot(pre_compile_sources, post_control_sources)
    except Exception as error:
        dump(build / "invalid_infrastructure.json", {
            "schema_version": "qwen35-n1-n2-codegen-infra-v1",
            "status": "INVALID",
            "stage": "post_control_toctou",
            "error": str(error),
            "traceback": traceback.format_exc(),
            "candidate_disposition": "NONE",
        })
    ...
    if control_failed:
        dump(build / "manifest.json", {
            ...
            "source_seal": {
                "pre_compile": pre_compile_sources,
                "post_compile": post_control_sources,
                "toctou_pass": True,
            },
    ...
    for model_id, candidate_id in CANDIDATES.items():
        for path_name in PATHS:
            try:
                ...
                if str(error).startswith("INVALID_INFRA:"):
                    dump(build / "invalid_infrastructure.json", {
                        "schema_version": "qwen35-n1-n2-codegen-infra-v1",
                        "status": "INVALID",
                        "stage": "candidate_import_or_identity",
                        "error": str(error),
                        "traceback": traceback.format_exc(),
                        "candidate_disposition": "NONE",
                    })
    ...
    try:
        post_compile_sources = verify_bound_sources()
        assert_same_source_snapshot(pre_compile_sources, post_compile_sources)
    except Exception as error:
        dump(build / "invalid_infrastructure.json", {
            "schema_version": "qwen35-n1-n2-codegen-infra-v1",
            "status": "INVALID",
            "stage": "post_candidate_toctou",
            "error": str(error),
            "traceback": traceback.format_exc(),
            "candidate_disposition": "NONE",
        })
    ...
    manifest = {
        ...
        "experiment_identity": experiment_identity,
        "bound_sources": pre_compile_sources,
        "source_seal": {
            "pre_compile": pre_compile_sources,
            "post_compile": post_compile_sources,
            "toctou_pass": True,
            "runtime_origins_recorded_per_configuration": True,
        },
