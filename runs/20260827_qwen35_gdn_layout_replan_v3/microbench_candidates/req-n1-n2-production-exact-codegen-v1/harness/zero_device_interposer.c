 ...
 *
 * la_symbind64 observes cross-DSO bindings selected by the glibc dynamic
 * linker, including dynamic lookup bindings presented to the audit API.
 * CUDA's private function-pointer paths are covered separately by rebinding
...
unsigned int la_objopen(
    ...
    (void)cookie;
    if (lmid == LM_ID_BASE) {
        emit_pair("NAMESPACE", "BASE");
    } else {
        /* dlmopen-created namespaces have a loader-assigned non-base ID. */
        emit_pair("NAMESPACE", "NONBASE");
    }
