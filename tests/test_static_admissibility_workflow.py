...
from advance_run import validate_experiment_queue, validate_strict_ranking
...
def main() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        closed_request["catalog_resolution"].update({
            ...
            "reason": "test terminal static gate",
        })
        closed_request["result_binding"] = {
            "status": "BOUND",
            "evidence": [{"path": "result.json", "sha256": "0" * 64}],
            "model_reconciliation": {"status": "APPLIED"},
        }
        closed_request["promotion_disposition"] = {
            "status": "RUN_LOCAL", "reason": "not a promoted performance package",
        }
        write(queue_path, closed)
        closed_errors: list[str] = []
        validate_experiment_queue(closed, workload, closed_errors)
        assert closed_errors == [], closed_errors
        completed = subprocess.run([
            sys.executable, str(ROOT / "scripts/rank_experiments.py"), "--run", str(run),
        ], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        assert completed.returncode == 0, (completed.stdout, completed.stderr)
        ...
        validate_strict_ranking(run, closed_ranked, closed_ranking_errors)
        assert closed_ranking_errors == [], closed_ranking_errors
