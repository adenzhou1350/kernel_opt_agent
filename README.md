...
`scripts/community_atomic_dispatcher.py` is that single-entry launcher. It
revalidates combined authorization on the live claim-store host, derives the
...
claims the next frozen entry, launches it once, and records `/proc` argv,
executable, boot and process-start identity. Output and log paths must be new,
distinct and below the artifact root. A timeout or either launch crash window
is terminal and never retried. Any ordinary process exit—including zero—stays
